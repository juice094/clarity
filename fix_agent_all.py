...
# 1. Ensure StreamDelta import exists
if 'use crate::llm::StreamDelta;' not in content:
    content = content.replace(
        'use crate::error::{AgentError, ToolError};\n\nuse crate::memory::{Memory, MemoryStore, MemoryTicker};',
        'use crate::error::{AgentError, ToolError};\nuse crate::llm::StreamDelta;\nuse crate::memory::{Memory, MemoryStore, MemoryTicker};'
    )

# 2. Ensure LlmProvider trait signature is updated
content = content.replace(
...
# 3. Update MockLlm::stream if not already
content = content.replace(
    '''    fn stream(
        &self,
        _messages: &[Message],
...
# 6. Update test mocks - MockLlmWithToolCall occurrences
# Replace all remaining old MockLlmWithToolCall stream methods
old_mock_stream = '''            fn stream(
                &self,
                _messages: &[Message],
                _tools: &Value,
            ) -> Result<tokio::sync::mpsc::Receiver<Result<String, AgentError>>, AgentError> {
...
content = content.replace(old_mock_stream, new_mock_stream)
...
with open(path, 'r', encoding='utf-8') as f:
    verify = f.read()
assert 'use crate::llm::StreamDelta;' in verify, "Import missing"
assert 'Result<StreamDelta, AgentError>' in verify, "StreamDelta type missing"
assert 'stream_tool_calls' in verify, "run_streaming not updated"
print('agent/mod.rs updated successfully')
