...
- **23 个 crate 目录 = 22 个活跃 workspace 成员 + 1 个归档 crate**（`clarity-tauri` 被 workspace 排除）。workspace 另含 `tests/integration` 集成测试 crate。
  - `clarity-slint` 是 workspace 成员但**不参与默认 CI**（实验性 Slint 前端）。
  - `clarity-mobile-core` 是新增 workspace 成员，负责 Android/iOS 的 UniFFI 桥接。
  - `clarity-openclaw` 负责 OpenClaw/KimiClaw 协议客户端与设备身份。
  - `clarity-thread-store`、`clarity-rollout` 负责 Thread/Session 生命周期管理。
...
| 文件 | 作用 |
|------|------|
| `Cargo.toml` | Workspace 配置、共享依赖、lint、profile。`members = ["crates/*", "tests/integration"]`，`exclude = ["crates/clarity-tauri"]`。 |
...
- `cargo run -p clarity-slint` — 实验性 Slint 前端。
...
├── crates/                 # 23 个 crate 目录 = 22 个活跃 workspace 成员 + 1 归档
├── docs/                   # 架构、开发、安全、规划文档
├── examples/               # 独立示例脚本
...
### 6.2 当前测试基线（2026-06-25，实机验证）

| 测试类型 | 通过 | 失败 | 忽略 |
|----------|------|------|------|
| `cargo test --workspace --lib --exclude clarity-slint` | 1554 | 0 | 0 |
| `cargo test --workspace --bins --exclude clarity-slint` | 275 | 0 | 0 |
...
Scope：`core`、`memory`、`gateway`、`egui`、`tui`、`claw`、`wire`、`headless`、`mobile`、`ci`、`docs`。
...
- **Pretext 文字测量与三栏布局**：egui 接入 `pretext-core` / `pretext-fontdb`，以 egui 字体栈作为 measurement backend；形成左 icon rail + 中主舞台 + 右工具 rail 的稳定布局；`MessageBubble` 与富文本段落已 pretext-aware。23 样本对齐回归测试与 1000 条消息 release 性能基准通过（聚合高度偏差 ≈ 1.45%，estimate ≈ 74.4 µs/msg，render ≈ 135.7 µs/msg）。
- **状态机与设计系统**：遗留 boolean modal / turn / expansion 标志统一迁移到 `ViewState`；egui 全局 `#![allow(dead_code)]` 已移除；右 rail 全部卡片与关键 widgets 使用 `design_system` 语义原语。
- **项目模型与上下文驱动**：`Session` 支持 `project_id` / `context` / `lifecycle` / `archived`；导航树按项目真实分组；归档会话可还原；无项目会话单独显示在 `Chats` 分组。
...
- **外部消息通道**：`clarity-channels` 当前仅实现 WeChat iLink（`chkit`）；Discord / Slack / Telegram 因上游 `rustls-webpki` 问题默认禁用；Webhook 默认可用。该 crate 是通道抽象而非完整多通道 Bot 矩阵。
- **移动端完整 UI**：仅 Rust FFI 核心落地，Android / iOS 完整 UI 仍在 `mobile/` 与 `docs/mobile-architecture.md` 路线图中。
- **Slint 前端**：`clarity-slint` 为实验栈，不参与默认 CI。
...
- `cargo fmt --all -- --check` 已清理（见 §6.2 基线）；提交前仍需保证格式化通过。
...
1. **新增/删除 crate 后**，必须同步更新以下文件中的 crate 拓扑：
   ...
   - `docs/architecture/ARCHITECTURE.md` §Project Layout
...
[Experimental / Utility]
  ├─ clarity-slint         (bin)  实验性 Slint GUI（依赖 contract + wire，不依赖 core）
