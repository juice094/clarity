...
- **20 个 crate 目录 = 19 个活跃 crate + 1 个归档 crate**（`clarity-tauri` 被 workspace 排除）。`clarity-thread-store`、`clarity-rollout` 负责 Thread/Session 生命周期管理（API 设计受 Codex 启发，实现为原创代码）。
...
### 6.2 当前测试基线（2026-06-18）

| 测试类型 | 通过 | 失败 | 忽略 |
|----------|------|------|------|
| `cargo test --workspace --lib --exclude clarity-slint` | 1220 | 0 | 8 |
| `cargo test --workspace --bins --exclude clarity-slint` | 186 | 0 | 2 |
...
> 注：`cargo test --workspace --lib --exclude clarity-slint` 包含 `tests/integration` crate 的 26 个集成测试；按 crate 拆分见 `docs/planning/PROJECT_STATUS.md` §1。
