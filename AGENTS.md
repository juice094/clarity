...
- **19 个活跃 crate + 1 个归档 crate**（`clarity-tauri` 被 workspace 排除）。新增 `clarity-thread-store`、`clarity-rollout`，用于从 Codex 迁移 Thread/Session 生命周期管理。
...
```text
contract ← {wire, memory, mcp, llm, tools, channels, thread-store} ← core ← {gateway, egui, tui, claw, headless}
                    ↑                                                        ↑
                 rollout                                               subagents（消费 core）
                                                                       telemetry（横切关注）
```
...
| `clarity-thread-store` | lib | Thread 持久化抽象：`ThreadStore` trait，从 Codex 迁移 |
| `clarity-rollout` | lib | JSONL rollout 持久化：事件日志、压缩、回放，从 Codex 迁移 |
...
> **新增说明**：`clarity-thread-store` 与 `clarity-rollout` 的 API 设计移植自 OpenAI Codex（Apache-2.0），并在 AGPL-3.0-or-later 下发布；相关 crate 的 `NOTICES.md` 保留了原始授权声明。
...
| `.github/workflows/ci.yml` | 10-job CI：check / test / integration / binary / doc-test / clippy / fmt / audit / doc-guard / coverage |
| `.github/workflows/release.yml` | Tag 触发 release，产出 Windows `.msi`/`.exe`、Linux binary、SHA256 校验 |
| `scripts/verify.ps1` | PowerShell 一键验收：README+AGENTS 存在性、编译、测试、Clippy、格式化 |
...
| 集成测试 | `cargo test -p clarity-integration-tests --lib` | adaptive_loop / session_v2_migration / telemetry_end_to_end |
...
- **S6 Pretext 三栏布局迁移**：左 icon rail + 中主舞台 + 右工具 rail 的单页面外壳已落地；右 rail 卡片（Status / Tools / Subagents / Memory）已接入；`legacy/task.rs` 与 `legacy/team.rs` 已删除。
