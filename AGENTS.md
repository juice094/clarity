...
- **22 个 crate 目录 = 21 个 workspace 成员 + 1 个归档 crate**（`clarity-tauri` 被 workspace 排除）。
...
- **定位边界**：聚焦编码/工程工作流的本地 AI 协作者。无原生消息通道客户端、无 Voice/Canvas、无完整移动端 UI（仅 Rust FFI 核心）。
...
├── crates/                 # 22 个 crate 目录 = 21 workspace 成员 + 1 归档
├── docs/                   # 架构、开发、安全、规划文档
├── examples/               # 独立示例脚本
