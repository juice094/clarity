...
- **20 个 crate 目录 = 19 个活跃 workspace 成员 + 1 个归档 crate**（`clarity-tauri` 被 workspace 排除）。workspace 另含 `tests/integration` 集成测试 crate。
...
contract
    ▲
    ├── {wire, memory, mcp, llm, tools, channels, secrets, rollout}
...
- `cargo fmt --all -- --check` 已清理；提交前仍需保证格式化通过。
...
[Experimental / Utility]
  ...
  ├─ clarity-anthropic-proxy (bin) Anthropic Messages API 网关（默认 DeepSeek device）
