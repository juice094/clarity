...
pub const ICON_BOT: &str = "\u{e1bb}"; // Lucide: Bot
...
pub struct Theme {
    ...
    // --- Spacing (8 px baseline grid: 4/8/12/16/20/24/32/40 px) ---
    pub space_4: f32,
    pub space_8: f32,
    pub space_12: f32,
    pub space_16: f32,
    pub space_20: f32,
    pub space_24: f32,
    /// Viewport-edge margin for modals and overlays (8× baseline).
    pub space_32: f32,
    /// Section-level spacing (5× baseline, for large block separators / empty states).
    pub space_40: f32,

    // --- Radius (8/16/28/36/9999 px scale) ---
    pub radius_sm: f32,
    pub radius_md: f32,
    pub radius_lg: f32,
    pub radius_xl: f32,
    ...
    // --- Composer (Kimi-style floating input card) ---
    pub composer_radius: f32,
    pub composer_shadow: egui::Shadow,
    ...
    // --- Button heights ---
    /// Small button height (e.g. toolbar tags).
    pub button_height_sm: f32,
    /// Medium button height (default action buttons).
...
fn shared_tokens(overlay_base: egui::Color32) -> SharedTokens {
    ...
    SharedTokens {
        ...
        composer_shadow: egui::Shadow {
            offset: [0, 4],
            blur: 24,
            spread: 0,
            color: rgba(0, 0, 0, shadow_alpha(0.35)),
        },
        duration_fast: 0.10,
...
struct SharedTokens {
    ...
    button_height_sm: f32,
    button_height_md: f32,
    button_height_lg: f32,
...
impl Theme {
    ...
    fn from_palette(palette: Palette, font_body: &str, font_mono: &str) -> Self {
        ...
        Self {
            ...
            composer_radius: t.composer_radius,
            composer_shadow: t.composer_shadow,
            ...
            button_height_sm: t.button_height_sm,
            button_height_md: t.button_height_md,
            button_height_lg: t.button_height_lg,
    ...
    /// Resolve a semantic animation speed to seconds.
    pub fn animation_duration(&self, speed: crate::animation::AnimationSpeed) -> f32 {
        use crate::animation::AnimationSpeed;
        match speed {
            AnimationSpeed::Fast => self.duration_fast,
            AnimationSpeed::Normal => self.duration_normal,
            AnimationSpeed::Slow => self.duration_slow,
        }
    }

    /// Animate a boolean state using theme duration.
    ///
    /// Returns a factor in `[0.0, 1.0]` that moves from `0.0` when `target` is
    /// `false` to `1.0` when `target` is `true`.
    pub fn animate_bool(
        &self,
        ctx: &egui::Context,
        id: egui::Id,
        target: bool,
        speed: crate::animation::AnimationSpeed,
    ) -> f32 {
        ctx.animate_bool_with_time(id, target, self.animation_duration(speed))
    }

    /// Animate a boolean state with the default normal speed.
    pub fn animate_bool_normal(&self, ctx: &egui::Context, id: egui::Id, target: bool) -> f32 {
        self.animate_bool(ctx, id, target, crate::animation::AnimationSpeed::Normal)
    }

    /// Animate a float target using theme duration.
    ///
    /// Returns a factor that moves toward `target` (usually `0.0` or `1.0`).
    pub fn animate_value(
        ...
        ctx.animate_value_with_time(id, target, self.animation_duration(speed))
    }

    /// Frame for elevated cards (dashboard widgets, conversation cards).
    pub fn frame_card(&self) -> egui::Frame {
        egui::Frame::new()
            .fill(self.surface)
            .stroke(egui::Stroke::new(0.5, self.border))
            .corner_radius(egui::CornerRadius::same(self.radius_md as u8))
            .shadow(self.shadow_card)
    }

    /// Frame for panels (sidebars, rails, input bar containers).
    ...
    /// Frame for modal/palette/floating surfaces.
    pub fn frame_modal(&self) -> egui::Frame {
        egui::Frame::new()
            .fill(self.bg_elevated)
            .stroke(egui::Stroke::new(1.0, self.border))
            .corner_radius(egui::CornerRadius::same(self.radius_md as u8))
            .shadow(self.shadow_modal)
    }

    /// Frame for toast notifications.
    pub fn frame_toast(&self) -> egui::Frame {
        egui::Frame::new()
            .fill(self.surface_strong)
            .stroke(egui::Stroke::new(1.0, self.border))
            .corner_radius(egui::CornerRadius::same(self.radius_md as u8))
            .shadow(self.shadow_toast)
    }
}
...
impl Theme {
    ...
    pub fn primary_button<'a>(&self, text: &'a str) -> Button<'a> {
        Button::new(text).variant(ButtonVariant::Primary)
    }

    /// Builds a secondary button widget.
    pub fn secondary_button<'a>(&self, text: &'a str) -> Button<'a> {
        Button::new(text).variant(ButtonVariant::Secondary)
    }

    /// Builds a ghost button widget.
    pub fn ghost_button<'a>(&self, text: &'a str) -> Button<'a> {
        Button::new(text).variant(ButtonVariant::Ghost)
    }
    ...
    pub fn with_font_scale(mut self, scale: f32) -> Self {
        ...
        // Button heights track the body font size so hit targets stay proportional.
        self.button_height_sm *= scale;
        self.button_height_md *= scale;
        self.button_height_lg *= scale;
        self
    }
    ...
    pub fn line_height(&self, ctx: &egui::Context) -> f32 {
        ...
        ctx.fonts_mut(|f| f.row_height(&font_id))
    }
...
pub fn rgba(r: u8, g: u8, b: u8, a: f32) -> egui::Color32 {
    let a = (a * 255.0).clamp(0.0, 255.0) as u8;
...
mod tests {
    ...
    #[test]
    fn button_height_tokens_are_ordered() {
        let t = Theme::dark();
        assert!(t.button_height_sm > 0.0);
        assert!(t.button_height_md > t.button_height_sm);
        assert!(t.button_height_lg > t.button_height_md);
    }
