...
//!
//! ponytail: moved from `clarity-egui` during P1c so `clarity-apps` can own the
//! Settings surfaces without depending on the egui host. The registry still
//! reads/writes the same filesystem paths, so existing user configs are preserved.
...
impl Default for ProviderRegistry {
    /// Returns an empty registry with no built-ins and no custom providers.
    ///
    /// Useful for tests and headless contexts that do not need provider lookup.
    fn default() -> Self {
        Self {
...
"#;
    ...
    #[test]
    fn test_auth_mode_password_requires_mobile() {
        let mut def = deepseek_device_def();
        def.auth_mode = AuthMode::Password;
        def.mobile = "".into();
        def.password_enc = Some("encrypted".into());
        let result = def.to_deepseek_device_provider("deepseek-chat");
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("mobile"));
    }

    #[test]
    fn test_to_deepseek_device_provider_non_deepseek_fails() {
        let def = ProviderDefinition {
            id: "openai".into(),
            ..Default::default()
        };
        let result = def.to_deepseek_device_provider("gpt-4o");
        assert!(result.is_err());
    }

    #[test]
    fn test_to_deepseek_device_provider_token_mode_needs_token() {
        let def = deepseek_device_def();
        let result = def.to_deepseek_device_provider("deepseek-chat");
        assert!(result.is_err());
    }
