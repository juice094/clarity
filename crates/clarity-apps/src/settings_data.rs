#![allow(missing_docs)]
//! Persisted GUI settings and OpenClaw connection data.
//!
//! These types were previously in `clarity-egui` and have been sunk to
//! `clarity-apps` during P1c so that the Settings sub-application can live
//! independently of the egui host. Encryption still uses `clarity_llm` so
//! settings files remain compatible.

use clarity_contract::settings::{
...
// ============================================================================
// OpenClaw URL helpers
// ============================================================================

/// Normalize a Gateway URL for comparison by stripping trailing slashes and
/// forcing a lowercase `ws://` or `wss://` scheme prefix.
...
/// Ensure a URL uses the WebSocket scheme for OpenClaw connections.
...
"#;
    ...
    #[test]
    fn test_normalize_gateway_url_strips_schemes() {
        assert_eq!(
            normalize_gateway_url("wss://Example.COM:18789/"),
            "ws://example.com:18789"
        );
        assert_eq!(
            normalize_gateway_url("https://example.com:18789"),
            "ws://example.com:18789"
        );
        assert_eq!(
            normalize_gateway_url("example.com:18789"),
            "ws://example.com:18789"
        );
    }

    #[test]
    fn test_to_ws_url_converts_schemes() {
        assert_eq!(to_ws_url("example.com"), "ws://example.com");
        assert_eq!(to_ws_url("https://example.com"), "wss://example.com");
        assert_eq!(to_ws_url("wss://example.com"), "wss://example.com");
    }
