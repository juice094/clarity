...
OpenClaw Gateways, as observed while wiring `clarity-openclaw` to remote
...
`clarity-openclaw` supports three `ClawAuth` variants:
...
header. `clarity-openclaw` therefore sends:
...
in every chunk. `clarity-openclaw` computes the incremental delta so the UI
