...
impl DeviceIdentity {
    ...
    pub fn generate_unpersisted() -> Self {
    ...
    pub fn load_or_generate() -> Result<Self, String> {
        Self::load_or_generate_at(device_identity_path()?)
    }

    /// Load an existing identity from the given path or generate a new one
    /// and persist it there.
    pub fn load_or_generate_at(path: impl AsRef<std::path::Path>) -> Result<Self, String> {
        ...
        identity.write_to_path(path)?;
        Ok(identity)
    }

    /// Load an identity from its JSON file representation.
    fn from_json(file: &str) -> Result<Self, String> {
    ...
    /// Persist this identity to the given path.
    pub fn write_to_path(&self, path: impl AsRef<std::path::Path>) -> Result<(), String> {
        let path = path.as_ref();
...
/// Load a previously-saved paired device token from the default location, if any.
///
...
pub fn load_paired_token() -> Result<Option<PairedToken>, String> {
    load_paired_token_at(device_token_path()?)
}

/// Load a previously-saved paired device token from the given path, if any.
pub fn load_paired_token_at(
    path: impl AsRef<std::path::Path>,
) -> Result<Option<PairedToken>, String> {
    let path = path.as_ref();
    if !path.exists() {
        return Ok(None);
    }
    let raw = std::fs::read_to_string(path).map_err(|e| format!("read token file: {}", e))?;
    parse_paired_token(&raw)
}

/// Persist a paired device token to the default location for later automatic reconnection.
pub fn save_paired_token(record: &PairedToken) -> Result<(), String> {
    save_paired_token_at(device_token_path()?, record)
}

/// Persist a paired device token to the given path.
pub fn save_paired_token_at(
    path: impl AsRef<std::path::Path>,
    record: &PairedToken,
) -> Result<(), String> {
    let path = path.as_ref();
...
fn parse_paired_token(raw: &str) -> Result<Option<PairedToken>, String> {
