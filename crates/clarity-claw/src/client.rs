...
//! 4. Reply:  `{"type":"res","id":"uuid","ok":true,"result":{...}}` or
...
async fn run_single_connection(
    ...
    fn build_connect_req(
        ...
        let mut auth_obj = serde_json::json!({ "token": token });
        if let ClawAuth::TokenWithDevice { device, .. } | ClawAuth::DevicePaired { device, .. } =
            auth
        {
            auth_obj["device_identity"] = serde_json::json!(device.public_key());
        }
            ...
            cmd = async_rx.recv() => {
                match cmd {
                    ...
                    Some(ClawCommand::RequestPairing {
                        ...
                        tracing::info!(request = %req, "OpenClaw sending device.pair.request");
                        if let Err(e) = write.send(Message::Text(req.to_string())).await {
                        ...
                        if let Ok(val) = serde_json::from_str::<serde_json::Value>(&text) {
                            ...
                            match msg_type {
                                "res" => {
                                    ...
                                    // OpenClaw returns `result` on success and `error` on failure.
                                    // Fall back to legacy `payload` for non-OpenClaw Gateways.
                                    let payload = if ok {
                                        val.get("result")
                                    ...
                                    let method = pending_methods.get(&id).cloned();
                                    tracing::info!(
                                        id = %id,
                                        method = ?method,
                                        ok = ok,
...
fn extract_messages(payload: &serde_json::Value) -> Option<Vec<GatewayMessage>> {
    ...
    if let Some(threads) = payload.get("threads").and_then(|v| v.as_array())
        && let Some(thread) = threads.first()
        && let Some(msgs) = thread.get("messages").and_then(|v| v.as_array())
    {
