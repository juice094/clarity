...
impl ToolRegistry {
    ...
    pub fn get_tool_schemas(&self) -> Result<Value, AgentError> {
        ...
        let schemas: Vec<Value> = tools.values().map(|tool| {
            json!({
                "type": "function",
                "function": {
                    "name": tool.name(),
    ...
    pub fn get_tool_definitions(&self) -> Result<Value, AgentError> {
        ...
        for tool in tools.values() {
            definitions.insert(tool.name().to_string(), json!({
                "description": tool.description(),
                "parameters": tool.parameters()
            }));
        }
    ...
    pub async fn execute(
        ...
        let tool = self.get(name).map_err(|e| {
            ToolError::execution_failed(format!("Registry error: {}", e))
        })?.ok_or_else(|| {
