...
impl Agent {
    ...
    /// automatically wrapped in a [`ReliableProvider`](clarity_contract::ReliableProvider)
    ...
    pub fn with_fallback_llms(self, fallbacks: Vec<Arc<dyn LlmProvider>>) -> Self {
        {
            ...
            if let Some(ref existing) = inner.llm {
                ...
                inner.llm = Some(Arc::new(clarity_contract::ReliableProvider::new(providers)));
            }
        }
        self
    }
    ...
    pub fn with_llm(self, llm: Arc<dyn LlmProvider>) -> Self {
        {
            ...
            let llm = if inner.fallback_llms.is_empty() {
                ...
                providers.extend(inner.fallback_llms.clone());
                Arc::new(clarity_contract::ReliableProvider::new(providers))
            };
            inner.llm = Some(llm);
