...
pub fn load_bundle(root: impl AsRef<Path>) -> OkfResult<OkfBundle> {
    let root = root.as_ref().canonicalize()?;
    let mut concepts = HashMap::new();
    let mut warnings = Vec::new();
    load_recursive(&root, &root, &mut concepts, &mut warnings)?;
    Ok(OkfBundle {
        root,
        concepts,
        warnings,
...
fn load_recursive(
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            load_recursive(bundle_root, &path, concepts, warnings)?;
        } else if path.extension().and_then(|s| s.to_str()) == Some("md") {
            match load_concept(bundle_root, &path) {
                Ok(concept) => {
                    concepts.insert(concept.id.clone(), concept);
                }
                Err(e) => {
                    let msg = format!("Skipping non-compliant OKF file {}: {}", path.display(), e);
                    tracing::warn!("{}", msg);
                    warnings.push(msg);
                }
            }
...
/// field.
pub fn load_concept(bundle_root: &Path, path: &Path) -> OkfResult<OkfConcept> {
...
pub fn parse_concept(id: impl Into<String>, content: &str) -> OkfResult<OkfConcept> {
    let id = id.into();
...
"#;
        let concept = parse_concept("metrics/wau", content).unwrap();
    ...
    fn test_parse_reserved_index_without_frontmatter() {
        ...
        let concept = parse_concept("index", content).unwrap();
        assert!(concept.is_reserved);
    ...
    fn test_parse_missing_frontmatter_fails() {
        ...
        let result = parse_concept("concept", content);
...
"#;
    ...
    fn test_load_bundle_from_dir() {
        ...
        let outgoing = bundle.outgoing_links("index");
