...
pub fn update_and_measure(app: &mut App, ctx: &egui::Context) -> LayoutMetrics {
    ...
    if metrics.content_too_narrow(app.context.ui_store.theme.content_min_width) {
...
fn apply_responsive_breakpoints(app: &mut App, ctx: &egui::Context) {
    ...
    if let Some(last_width) = app.last_frame_width {
        ...
        if last_width >= app.context.ui_store.theme.breakpoint_wide
            && current_width < app.context.ui_store.theme.breakpoint_wide
        {
            app.collapse_right_rail();
        }
        // Below medium breakpoint: collapse legacy right panels only.
        if last_width >= app.context.ui_store.theme.breakpoint_medium
            && current_width < app.context.ui_store.theme.breakpoint_medium
        {
            app.view_state.right = None;
        }
        // Below compact breakpoint: hide the right utility rail only.
        if last_width >= app.context.ui_store.theme.breakpoint_compact
            && current_width < app.context.ui_store.theme.breakpoint_compact
        {
            app.collapse_right_rail();
        }
    }
...
fn compute_metrics(app: &App, ctx: &egui::Context) -> LayoutMetrics {
    ...
    let right_w = app
        ...
        .ui_store
        .right_rail_width
        .unwrap_or(app.context.ui_store.theme.size_panel_right);
    // Use the animated left rail width for smooth expand/collapse transitions.
    let left_w = app.effective_left_rail_width(ctx);
    compute_metrics_raw(
        ctx.input(|i| i.viewport_rect()).width(),
        left_w,
        right_w,
        app.is_right_rail_visible(),
    )
}
