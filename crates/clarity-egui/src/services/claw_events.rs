...
impl App {
    ...
    pub(crate) fn manage_claw_connection(&mut self) {
        ...
        let Some(conn) = self.device_state.connection(&picked_id) else {
            ...
            match event {
                crate::claw::ClawEvent::Connected {
                    ...
                    if let Some(session) = self.session_store.active_session_mut() {
                        if let SessionContext::Claw { affinity, .. } = &mut session.context {
                            *affinity = DeviceAffinity::Specific(self.claw_ws_device_id.clone());
                        }
                    }
                ...
                crate::claw::ClawEvent::StreamChunk(text) => {
                    if !text.trim().is_empty() {
                        ...
                        let _ = self.ui_tx.send(UiEvent::Chunk { session_id, text });
                    }
                }
                crate::claw::ClawEvent::Done => {
                    let session_id = self
                        ...
                        .take()
                        .unwrap_or_else(|| self.session_store.active_session_id.clone());
                    let _ = self.ui_tx.send(UiEvent::Done { session_id });
                }
                ...
                crate::claw::ClawEvent::Error(e) => {
                    ...
                    self.claw_ws_device_id.clear();
                    if !failed_device.is_empty() {
                        let (count, _next) = self.ui_store.record_connect_failure(&failed_device);
                        if count >= 5 {
    ...
    pub(crate) fn drain_pairing_responses(&mut self) {
        let Some(client) = self.claw_pairing_client.as_ref() else {
            return;
        };
