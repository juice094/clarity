...
pub fn render_subagent_progress(app: &mut App, ui: &mut egui::Ui) {
    if app.parallel_batches.is_empty() {
    ...
    for (idx, batch) in app.parallel_batches.iter().enumerate() {
                            ...
                            ui.label(
                                egui::RichText::new(&batch.status)
                                    ...
                                    .color(match batch.status.as_str() {
                                        "Running" => app.theme.status_online,
                                        "Completed" => app.theme.status_online,
                                        "Failed" => app.theme.danger,
        ...
        for idx in to_remove.into_iter().rev() {
            app.parallel_batches.remove(idx);
        }
}
