...
pub fn render_task_create_modal(app: &mut App, ctx: &egui::Context) {
    if !app.task_create_modal_open { return; }
    ...
    egui::Window::new("Create Task")
        ...
        .frame(egui::Frame::window(&ctx.style()).fill(app.theme.surface).corner_radius(egui::CornerRadius::same(app.theme.radius_md as u8)))
        .show(ctx, |ui| {
            ui.set_min_width(360.0);
            ui.set_max_width(480.0);
            ui.heading(egui::RichText::new("New Background Task").color(app.theme.text));
            ui.add_space(12.0);
            ui.label(egui::RichText::new("Name").size(12.0).color(app.theme.text).strong());
            ui.add(egui::TextEdit::singleline(&mut app.task_create_name).hint_text("Task name"));
            ui.add_space(8.0);
            ui.label(egui::RichText::new("Description").size(12.0).color(app.theme.text).strong());
            ui.add(egui::TextEdit::singleline(&mut app.task_create_desc).hint_text("Short description"));
            ui.add_space(8.0);
            ui.label(egui::RichText::new("Prompt").size(12.0).color(app.theme.text).strong());
            ui.add_sized(egui::vec2(ui.available_width(), 80.0), egui::TextEdit::multiline(&mut app.task_create_prompt).hint_text("Agent prompt..."));
            ui.add_space(8.0);
            ui.label(egui::RichText::new("Priority").size(12.0).color(app.theme.text).strong());
            ...
            ui.horizontal(|ui| {
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let can_create = !app.task_create_name.trim().is_empty() && !app.task_create_prompt.trim().is_empty();
                    let create_btn = ui.add_sized(egui::vec2(80.0, 32.0), egui::Button::new(egui::RichText::new("Create").size(13.0).color(app.theme.text)).fill(if can_create { app.theme.accent } else { app.theme.bg_elevated }));
                    if create_btn.clicked() && can_create {
                        created = true;
                    }
                    if ui.add_sized(egui::vec2(80.0, 32.0), egui::Button::new(egui::RichText::new("Cancel").size(13.0).color(app.theme.text)).fill(app.theme.border)).clicked() {
