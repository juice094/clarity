...
pub fn render_mcp_panel(app: &mut App, ctx: &egui::Context) {
    ...
    egui::Window::new("MCP Servers")
        ...
        .frame(egui::Frame::window(&ctx.style()).fill(app.theme.surface).corner_radius(egui::CornerRadius::same(app.theme.radius_lg as u8)))
        .show(ctx, |ui| {
            ...
            if let Some(ref mut config) = config_opt {
                ...
                ui.vertical_centered(|ui| {
                    ui.add_space(40.0);
                    ui.label(egui::RichText::new("No MCP config found").size(13.0).color(app.theme.text_dim));
                    ui.add_space(8.0);
                    if ui.add(egui::Button::new(egui::RichText::new("Create Config").size(13.0).color(app.theme.text)).fill(app.theme.accent).min_size(egui::vec2(140.0, 36.0))).clicked() {
                        create_clicked = true;
                    }
                });
            }
    ...
    if save_clicked {
        if let Some(ref mut config) = config_opt {
            match crate::ui::mcp_panel::save_mcp_config(config) {
                ...
                Err(e) => app.push_toast(format!("Save failed: {}", e), crate::ui::types::ToastLevel::Error),
            }
        }
    }
    if cancel_clicked {
