...
使用 Rust 构建。核心引擎和 CLI 工具以**单二进制文件**分发，**零外部运行时依赖**（无需 Python、Node.js 或 Ollama）。桌面 GUI（eframe/egui）是纯 Rust 实现，零 Web 依赖。
...
| **多入口** | 同一智能体核心，五种界面：TUI、桌面 GUI、Web IDE、无头 CLI、系统托盘。 |
...
```bash
# 运行完整验证套件（CI 执行的）
cargo test --workspace --lib --exclude clarity-slint      # 1087 测试，0 失败，8 忽略
