...
本文档包含两部分：
1. **当前架构** — Clarity 已实现的三层运行时（claw/window/cli）
2. **未来架构** — 消息平台渠道集成设计（Telegram/Discord/Slack 等），目标支持 **15+ 平台**

> ⚠️ 渠道集成（Channels）当前处于设计阶段，尚未实现。Clarity v0.1.1 仅支持 Web UI 作为人机交互界面。

---

## 2. 当前三层运行时架构（v0.1.1 已实现）

Clarity 采用 **claw / window / cli** 三层差异化架构：
...
| 维度 | claw | window | cli |
|------|------|--------|-----|
| **形态** | 系统托盘图标 | 浏览器标签页 | 终端窗口 |
| **常驻性** | ✅ 开机自启，永远在线 | ⚠️ 按需打开 | ❌ 手动启动 |
| **交互深度** | 被动通知 | 中等（点击操作） | 完整（命令输入） |
| **典型场景** | 任务完成提醒、后台监控 | 文件浏览、配置管理 | 深度对话、批量操作 |
| **会话隔离** | 共享主会话 | 独立 Web 会话 | 独立 TUI 会话 |
| **技术栈** | tao + tray-icon | axum + static HTML | ratatui |
...
│
├─ 启动时
│   └─ 读取 ~/.config/clarity/config.toml
│   └─ 连接 Gateway WebSocket (/ws)
│
├─ 运行时
│   └─ 轮询 .clarity/tasks/ 目录状态
│   └─ 通过 OS 通知推送任务事件
│   └─ 点击托盘图标 → 弹出任务列表面板
│
├─ 通知类型
