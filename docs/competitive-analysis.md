...
| 功能模块 | **Clarity** | **claude-code-rust** | **5ire** | **openclaw** | **zeroclaw** |
|:---------|:-----------:|:--------------------:|:--------:|:------------:|:------------:|
| **技术栈** | Rust | Rust | TS/Electron | Rust | Rust |
| **容器形态** | ✅ **claw(托盘)+window(Web)+cli(TUI)** | TUI+GUI+CLI | Electron 桌面 | CLI/TUI | CLI |
| **形态差异化** | ✅ **三层独立会话+差异化提示词** | ❌ 通用提示词 | ❌ 通用 | ❌ 通用 | ❌ 通用 |
| **常驻/后台** | ✅ **claw 系统托盘常驻** | ❌ | ⚠️ 桌面常驻 | ❌ | ❌ |
| **LLM 对话** | ✅ SSE 流式 | ✅ REPL + TUI | ✅ | ✅ | ✅ |
| **模型中立** | ✅ **多模型切换** | ⚠️ 单模型 | ❌ 单模型 | ⚠️ 单模型 | ⚠️ 单模型 |
| **MCP 工具** | ✅ **Client（动态加载）** | ❌ | ✅ Client | ❌ | ❌ |
| **文件操作** | ✅ 读/写/搜索/执行 | ✅ | ⚠️ 有限 | ⚠️ 有限 | ⚠️ 有限 |
| **本地记忆** | ✅ 会话持久化+devbase 日志 | ✅ Skill 系统 | ✅ 知识库 RAG | ❌ | ❌ |
